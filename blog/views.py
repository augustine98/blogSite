...
def comment(request, pk):
    ...
    if 'parent_id' in request.POST:
        comm = Comment.objects.create(body=_comment, parent=get_object_or_404(
            Comment, pk=request.POST.get('parent_id')), author=user, post=post)
    else:
        comm = Comment.objects.create(body=_comment, author=user, post=post)
    return JsonResponse({'date_posted': comm.date_posted.strftime("%B %d, %Y"), 'id': comm.id})
